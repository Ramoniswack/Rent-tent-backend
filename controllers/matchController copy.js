...
exports.likeUser = async (req, res) => {
  try {
    ...
    if (!match) {
      ...
      // Update the like status (both legacy and new fields)
      if (isUser1) {
  ...
  try {
    ...
    if (!match) {
      match = new Match({
        ...
        user1Liked: false,
        user2Liked: false,
        user1Status: isUser1 ? 'pass' : 'none',
        user2Status: isUser1 ? 'none' : 'pass'
      });
    } else {
      // Update the like status to false (both legacy and new fields)
      if (isUser1) {
        match.user1Liked = false;
        match.user1Status = 'pass';
      } else {
        match.user2Liked = false;
        match.user2Status = 'pass';
      }
      match.matched = false;
    }
...
exports.getLikes = async (req, res) => {
  try {
    ...
    // Support both new status fields and legacy liked fields
    const matches = await Match.find({
      $or: [
        // New status fields
        { user1: currentUserId, user2Status: 'like', user1Status: { $ne: 'like' }, matched: false },
        { user2: currentUserId, user1Status: 'like', user2Status: { $ne: 'like' }, matched: false },
        // Legacy liked fields (backward compatibility)
        { user1: currentUserId, user2Liked: true, user1Liked: false, matched: false },
        { user2: currentUserId, user1Liked: true, user2Liked: false, matched: false }
      ]
    })
    .populate('user1', '-password')
    .populate('user2', '-password')
...
// GET /api/matches/sent - Get users current user has liked but not matched yet
exports.getSentLikes = async (req, res) => {
  try {
    ...
    const matches = await Match.find({
      $or: [
        // New status fields
        { user1: currentUserId, user1Status: 'like', user2Status: { $ne: 'like' }, matched: false },
        { user2: currentUserId, user2Status: 'like', user1Status: { $ne: 'like' }, matched: false },
        // Legacy liked fields (backward compatibility)
        { user1: currentUserId, user1Liked: true, user2Liked: false, matched: false },
        { user2: currentUserId, user2Liked: true, user1Liked: false, matched: false }
      ]
    })
    .populate('user1', '-password')
    .populate('user2', '-password')
